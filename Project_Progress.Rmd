...
tr_test1 <- xgb.train(booster = "gbtree", objective = "binary:logistic", data = xgb_bm1[[1]], nrounds = 200, max_depth = 6, eval_metric = "logloss", n_estimators = 500, gamma = 0, alpha = 0)
...
```{r}
var1 <- true_split1[[2]] %>% select("MissingET.MET", "MissingET.Phi", "Lepton.PT", "Lepton.PT", "Lepton.Phi", "ScalarHT.HT", "Jet1.PT", "MissingET.Eta", "BJet.PT", "Lepton.Eta", "BJet.Eta","Signal") %>% sample_n(500)
col1 <- if_else(var1$Signal==0, "blue", "red")
var1 <- apply(var1, 2, scale)


t1 <- save_history(var1[,-11], guided_tour(ashapetour(cl = col1, alpha = 0.4, hull_var = c("blue", "red")), search_f = tourr:::search_better, alpha = pi/2, cooling = 0.5))

set.seed(100)
X11()
animate_xy(var1[,-11], guided_tour(index_f = ashapetour(cl = col1, alpha = 0.4, hull_var = c("blue", "red")), search_f = tourr:::search_better, alpha = pi/2, cooling = 0.5), col = col1)
X11()
...
m <- list(basisMatrix(1,2,10), basisMatrix(3,4,10))
indexList <- list(ashapetour(cl = col1, alpha = 0.4, hull_var = c("blue", "red")))
indexLabels <- list("alpha")
trace <- getTrace(select(var1, -"Signal"), m, indexList, indexLabels)
plotTrace(trace)
```

Find the most optimal parameter (DO NOT USE THIS JUST YET)
```{r}
...
new_param <- expand.grid(
  n_estimators = 500,
  nrounds = 500,
  eta = seq(from = 0.1, to = 0.4, by = 0.1),
  max_depth = 4:6, 
  eval_metric = "logloss",
...
pred_test <- predict(tr_test, xgb_bm1[[4]])
confusionMatrix(table(pred_test, part_split1[[2]]$Signal))

# train on original training set
tr_xgb <- xgb.train(params = best_param, 
          data = xgb_bm1[[1]], nrounds = xgbcv[[best_xgbcv]]$best_iteration)

# test error after prediction
xgbpred <- predict(tr_xgb, xgb_bm1[[2]])
```
