...
=======
  filter(MissingET.MET < 250) # Select events with MET >250 GeV
>>>>>>> 26b35f8055bd423430624744b2cdb8a0b01983c9
  return(data_rmv)
}
...
=======
  data1 = data1 %>% anti_join(data1_rmv[sample(seq(nrow(data2_rmv),nrow(data1_rmv))),])
>>>>>>> 26b35f8055bd423430624744b2cdb8a0b01983c9
...
=======
true_split2 <- true_partition(Data_Benchmark01)
>>>>>>> 26b35f8055bd423430624744b2cdb8a0b01983c9
...
tr_test1 <- xgb.train(booster = "gbtree", objective = "binary:logistic", data = xgb_bm1[[1]], nrounds = 200, max_depth = 6, eval_metric = "rmse")

pred_test1 <- predict(tr_test1, xgb_bm1[[2]])
check_data1 <- true_split1[[2]] %>% mutate(Signal_pred = pred_test1, Signal = as.factor(Signal))
...
tr_test <- xgb.train(booster = "gbtree", objective = "binary:logistic", data = xgb_bm1[[1]], nrounds = 200, max_depth = 6, eval_metric = "rmse")
pred_test <- predict(tr_test, xgb_bm1[[2]])
...
b1_vals <- Values(pred_test1, true_split1, Data_Benchmark01, 0.1593)
b1_vals
```

Visualisation using guided tours
```{r}
var1 <- true_split1[[2]] %>% select("MissingET.MET", "MissingET.Phi", "Lepton.PT", "Lepton.Phi", "ScalarHT.HT", "Jet1.PT", "MissingET.Eta", "BJet.PT", "Lepton.Eta", "BJet.Eta", "Signal") %>% sample_n(5000)
col1 <- if_else(var1$Signal==0, "blue", "red")

t1 <- save_history(select(var1, -"Signal"), guided_tour(ashapetour(cl = col1, alpha = 0.4, hull_var = c("blue", "red"))))
X11()
  animate_xy(select(var1, -"Signal"), guided_tour(ashapetour(cl = col1, alpha = 0.4, hull_var = c("blue", "red"))), col = col1, axes="bottomleft")
X11()
=======
confusionMatrix(table(pred_test, true_split1[[2]]$Signal))
>>>>>>> 26b35f8055bd423430624744b2cdb8a0b01983c9

# Save the tour plot
render(select(var1, -"Signal"), planned_tour(t1), display_xy(col = col1), "png", "/home/user1/Documents/Project/Created Figures/alpha.png")
```
...
h2o.xgb_param <- expand.grid(
  ntrees = seq(from = 400, to = 500, by = 50),
  max_depth = 5:7,
  col_sample_rate = c(0.5, 0.75, 1.0),
  learn_rate = seq(from = 0.2, to = 0.6, by = 0.1),
  stopping_metric = c("mse", "rmse", "logloss")
)
...
Turns out MSE was the better choice this time, so it may go back and forth, but it seems taking the mean squared error itself is most optimal for this problem.
...
tr_test2 <- xgb.train(booster = "gbtree", objective = "binary:logistic", data = xgb_bm2[[1]], nrounds = 200, max_depth = 6, eval_metric = "rmse")
pred_test2 <- predict(tr_test2, xgb_bm2[[2]])

check_data2 <- true_split2[[2]] %>% mutate(Signal_pred = pred_test2, Signal = as.factor(Signal))
...
b2_vals <- Values(pred_test2, true_split2, Data_Benchmark02, 2.877)
b2_vals
...
tr_test3 <- xgb.train(booster = "gbtree", objective = "binary:logistic", data = xgb_bm3[[1]], nrounds = 200, max_depth = 6, eval_metric = "rmse")
pred_test3 <- predict(tr_test3, xgb_bm3[[2]])

check_data3 <- true_split3[[2]] %>% mutate(Signal_pred = pred_test3, Signal = as.factor(Signal))
...
b3_vals <- Values(pred_test3, true_split3, Data_Benchmark01, 0.6571)
b3_vals
```
...
xgb.ggplot.importance(xgb.importance(model = tr_test2))
xgb.ggplot.importance(xgb.importance(model = tr_test3))
```
...
plot.roc(true_split2[[2]]$Signal, pred_test2, ci = TRUE, ci.alpha = 0.9,
         percent = TRUE, print.auc = TRUE, add = TRUE, col = "green4", print.auc.x = 70, print.auc.y = 45)
plot.roc(true_split3[[3]]$Signal, pred_test3, ci = TRUE, ci.alpha = 0.9,
         percent = TRUE, print.auc = TRUE, add = TRUE, col = "darkmagenta", print.auc.x = 70, print.auc.y = 35)
legend("bottomright", legend=c("Benchmark 1", "Benchmark 2", "Benchmark 3"), col = c("blue3", "green4", "darkmagenta"), pch = 15)
>>>>>>> 26b35f8055bd423430624744b2cdb8a0b01983c9
...
```{r}
<<<<<<< HEAD
test <- concaveman(as.matrix(var1))
plot(var1)
plot(test, add = TRUE)
```
=======
