...
perform_cv <- function(Parameters, Data){
  ...
  xgbcv <- 
    xgb.cv(params = Parameters[i,], data = Data[[3]], nrounds = Parameters[i,2],
           ...
           maximize = F)
  Result1[i,1] <- min(xgbcv$evaluation_log$test_logloss_mean)
  Result1[i,2] <- xgbcv$best_iteration
  rm(xgbcv)
  return(Result)
  }
}
...
```{r}
var1 <- true_split1[[2]] %>% mutate(Signal_pred = xgbpred1) %>% select("MissingET.MET", "MissingET.Phi", "Lepton.PT", "Lepton.PT", "Lepton.Phi", "ScalarHT.HT", "Jet1.PT", "MissingET.Eta", "BJet.PT", "Signal", "Signal_pred") %>% sample_n(NROW(true_split1[[2]])/10)
...
# False positive = Green/2 and False negative = red/3
var1 <- apply(var1[,-10], 2, scale)

# Save tour history
set.seed(1000)
t1 <- save_history(var1[,-10], guided_tour(ashapetour(cl = col_v1[,3], alpha = 0.4, hull_var = c("black")), search_f = tourr:::search_better, alpha = pi/2, cooling = 0.5), step_size = Inf)

X11()
animate_xy(var1[,-10], planned_tour(t1), col = col_v1[,3], fps = 50)
#dev.copy2pdf(file = "/home/user1/Desktop/Images for thesis/tourr/test.pdf") # is supposed to copy the plot in pdf
X11()

# Save the tour plot
render(var1[,-10], planned_tour(t1), display_xy(col = col_v1[,3]), frames = 50, "png", "/home/user1/Desktop/Images for thesis/tourr/bm1-%03d.png")
...
```{r}
var2 <- true_split2[[2]] %>% mutate(Signal_pred = xgbpred2) %>% select("MissingET.MET", "MissingET.Phi", "Lepton.PT", "Lepton.PT", "Lepton.Phi", "ScalarHT.HT", "Signal", "Signal_pred") %>% sample_n(NROW(true_split2[[2]])/10)
...
# False positive = Green/2 and False negative = red/3
var2 <- apply(var2[,-7], 2, scale)

# Save tour history
set.seed(1000)
t2 <- save_history(var2[,-7], guided_tour(ashapetour(cl = col_v2[,3], alpha = 0.4, hull_var = c("black")), search_f = tourr:::search_better, alpha = pi/2, cooling = 0.5), step_size = Inf)

X11()
animate_xy(var2[,-7], planned_tour(t2), col = col_v2[,3], fps = 50)
#dev.copy2pdf(file = "/home/user1/Desktop/Images for thesis/tourr/test.pdf") # is supposed to copy the plot in pdf
X11()

# Save the tour plot
render(var2[,-7], planned_tour(t2), display_xy(col = col_v2[,3]), frames = 50, "png", "/home/user1/Desktop/Images for thesis/tourr/bm2_1/bm2_1-%03d.png")
# pdf version actually captures the whole path
```
...
```{r}
var3 <- true_split3[[2]] %>% mutate(Signal_pred = xgbpred3) %>% select("MissingET.MET", "MissingET.Phi", "Lepton.PT", "Lepton.PT", "Lepton.Phi", "ScalarHT.HT", "Signal", "Signal_pred") %>% sample_n(NROW(true_split3[[2]])/10)
...
# False positive = Green/2 and False negative = red/3
var3 <- apply(var3[,-7], 2, scale)

# Save tour history
set.seed(1000)
t3 <- save_history(var3[,-7], guided_tour(ashapetour(cl = col_v3[,3], alpha = 0.4, hull_var = c("black")), search_f = tourr:::search_better, alpha = pi/2, cooling = 0.5), step_size = Inf)

X11()
animate_xy(var3[,-7], planned_tour(t3), col = col_v3[,3], fps = 50)
#dev.copy2pdf(file = "/home/user1/Desktop/Images for thesis/tourr/test.pdf") # is supposed to copy the plot in pdf
X11()

# Save the tour plot
render(var3[,-7], planned_tour(t3), display_xy(col = col_v3[,3]), frames = 50, "png", "/home/user1/Desktop/Images for thesis/tourr/bm3_1/bm3_1-%03d.png")
# pdf version actually captures the whole path
```
...
In_vals <- Values(check_data_I$Signal_pred, true_split_I, Data_Benchmark0_I, 4.001)
In_vals
```
...
```{r}
var_I <- true_split_I[[2]] %>% mutate(Signal_pred = xgbpred_I) %>% select("MissingET.MET", "MissingET.Phi", "Lepton.PT", "Lepton.PT", "Lepton.Phi", "ScalarHT.HT", "Signal", "Signal_pred") %>% sample_n(NROW(true_split_I[[2]])/10)
...
# False positive = Green/2 and False negative = red/3
var_I <- apply(var_I[,-7], 2, scale)

# Save tour history
set.seed(1000)
t_I <- save_history(var_I[,-7], guided_tour(ashapetour(cl = col_I[,3], alpha = 0.4, hull_var = c("black")), search_f = tourr:::search_better, alpha = pi/2, cooling = 0.5), step_size = Inf)

X11()
animate_xy(var_I[,-7], planned_tour(t_I), col = col_I[,3], fps = 50)
#dev.copy2pdf(file = "/home/user1/Desktop/Images for thesis/tourr/test.pdf") # is supposed to copy the plot in pdf
X11()

# Save the tour plot
render(var_I[,-7], planned_tour(t_I), display_xy(col = col_I[,3]), frames = 50, "png", "/home/user1/Desktop/Images for thesis/tourr/bm_I_1/bm_I_1-%03d.png")
# pdf version actually captures the whole path
```
...
```{r, message = False}
xgbcv_result_check <- perform_cv(new_param, xgb_bm_check)
# row/list number of the best xgbcv
best_xgbcv <- which.min(xgbcv_result_check[,1])
best_param <- new_param[best_xgbcv,]

# train on partitioned set
tr_test_check <- xgb.train(params = best_param, data = xgb_bm_check[[3]], nrounds = xgbcv_result_check[best_xgbcv,2])
...
# Save tour history
set.seed(1000)
t_check <- save_history(var_check[,-7], guided_tour(ashapetour(cl = col_check[,3], alpha = 0.4, hull_var = c("black")), search_f = tourr:::search_better, alpha = pi/2, cooling = 0.5), step_size = Inf)
...
```{r}
all_dir <- list.dirs(path = "/home/user1/Desktop/Images for thesis/tourr/", recursive = FALSE)
for (d in all_dir){
  name <- paste0("/home/user1/Desktop/Images for thesis", strsplit(d, "/")[[1]][3], ".gif")
