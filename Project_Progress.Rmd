...
new_param <- expand.grid(
  ...
  eta = seq(from = 0.1, to = 0.3, by = 0.05),
  max_depth = 4:8, 
  eval_metric = c("merror", "rmse", "mlogloss")
)
...
# Perform xgboost cv
xgbcv <- vector("list", NROW(new_param)) 
...
# extract minimum test MSE mean from each xgbcv
xgbcv_result <- vector("numeric", NROW(new_param)) 
for (i in 1:NROW(new_param)){
  xgbcv_result[[i]] <- min(xgbcv[[i]]$evaluation_log$test_merror_mean)
}

#### row/list number of the best xgbcv
best_xgbcv <- which.min(xgbcv_result)

best_param <- new_param[best_xgbcv,]

# train on partitioned set
tr_test <- xgb.train(params = best_param, data = xgb_tr, nrounds = xgbcv[[best_xgbcv]]$best_iteration)
...
set.seed(sample(1:1000000001, 1, replace=T))
tr_indx <- createDataPartition(Data_benchmark01$Signal, p = 2/3)$Resample1
true_train <- as.h2o(Data_benchmark01[tr_indx, ])
true_test <- as.h2o(Data_benchmark01[-tr_indx, ])

set.seed(sample(1:1000000001, 1, replace=T))
tr_indx2 <- createDataPartition(true_train$Signal, p = 0.75)$Resample1
part_train <- as.h2o(true_train[tr_indx2, ])
part_test <- as.h2o(true_test[-tr_indx2, ])
...
# Perform xgboost cv
xgb <- vector("list", NROW(new_param)) 
for (i in 1:NROW(new_param)){
