...
for (i in 4*NROW(new_param)/5:NROW(new_param)/5){
  xgbcv <- 
    xgb.cv(params = new_param[i,], data = xgb_tr, nrounds = new_param[i,1],
           nfold = 10, showsd = T, stratified = T, print_every_n = 10, early_stopping_round = 20,
           maximize = F)
...
Preds <- as.data.frame(h2o.predict(xgb, part_ts))

confusionMatrix(table(Preds$predict, part_test[1:length(Preds$predict),37]))
...
# Reduce sample size of stops to match background's (top) sample size
stopbm2 <- stopbm2 %>% anti_join(stopbm1_rmv[sample(seq(nrow(topData_rmv),nrow(stopbm2_rmv), by = 1)),])
...
true_train <- Data_benchmark02[tr_indx, ]
true_test <- Data_benchmark02[-tr_indx, ]
...
set.seed(sample(1:1000000001, 1, replace=T))
tr_indx <- createDataPartition(Data_benchmark01$Signal, p = 2/3)$Resample1
true_train <- Data_benchmark03[tr_indx, ]
true_test <- Data_benchmark03[-tr_indx, ]
