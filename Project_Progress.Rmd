...
Values <- function(pred, true, data, stopCS){ # data1 = predicted, data2 = true, data3 = combined data
  ...
  Nb = round((2500*137)*(3*confMat$table[1,2]/1000000))
  AMS = sqrt(2*(Ns+Nb+10)*log(1+(Ns/(Nb+10)))-Ns)
  
  Vals[[1]] = c("TP", confMat$table[2,2]/nrow(true[[2]]))
  Vals[[2]] = c("TN", confMat$table[1,1]/nrow(true[[2]]))
  Vals[[3]] = c("FP" , confMat$table[1,2]/nrow(true[[2]]))
  Vals[[4]] = c("FN", confMat$table[2,1]/nrow(true[[2]]))
  ...
  Vals[[7]] = c("SNR", confMat$table[2,2]/confMat$table[1,1])
  Vals[[8]] = c("Nb", Nb)
  Vals[[9]] = c("Ns", Ns)
  Vals[[10]] = c("AMS", AMS)
...
col_f1 <- if_else(false1$Signal_pred=="2", "blue", "red") # red is FN, blue is FP
...
X11()
animate_xy(false1[,-ncol(false1)], planned_tour(f1), col = col_test, fps = 50)
X11()

animation_falseVar(false1, "/home/user1/Desktop/false1.csv", f1)
```
...
htmlwidgets::saveWidget(as_widget(create_animation(t3, "/home/user1/Desktop/var3.csv", var3)), "/home/user1/Desktop/Images for thesis/tourr/bm3_test.html")
...
htmlwidgets::saveWidget(as_widget(create_animation_tr(t3, "/home/user1/Desktop/var3_tr.csv", var3_tr)), "/home/user1/Desktop/Images for thesis/tourr/bm3_test_tr.html")
...
```{r}
xgb.ggplot.importance(xgb.importance(model = tr_xgb1), top_n = 10)
xgb.ggplot.importance(xgb.importance(model = tr_xgb2), top_n = 10)
xgb.ggplot.importance(xgb.importance(model = tr_xgb3), top_n = 10)
```
...
```{r}
Benchmarks <- c("Outside_1", "Outside_2", "Outside_3", "Inside")
Stop_Mass <- c(1.275, 1.25, 1.225, 0.75)
Chi_Mass <- c(600, 400, 100, 1)
table1 <- cbind(Benchmarks, Stop_Mass, Chi_Mass)
colnames(table1) <- c("Model", "Stop Mass (TeV)", "Neutralino Mass (GeV)")

kable(table1, "latex", caption = "Chosen parameters for building classifiers", booktabs = T, linesep = "") %>%
