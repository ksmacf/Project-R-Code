...
new_param <- expand.grid(
  ...
  max_depth = 4:6, # first try said 5, consistent with the h2o version
  eval_metric = "logloss",
  alpha = c(0.1, 0.5, 1, 2, 5, 10) # first try said 1 is a good number, so test values closer to it
)
...
# Do a check to see what is a good cut-off
check <- part_split3[[2]] %>% mutate(Signal_pred = pred_test2, Signal = as.factor(Signal))
ggplot(check, aes(x = Signal_pred, fill = Signal)) +
  geom_histogram(bins = 50, alpha=.5, position="identity") +
  scale_y_continuous(trans = 'log10') +
...
for (i in 1:NROW(check_data2$Signal_pred)){
  if (check_data2$Signal_pred[i] > 0.6){
       check_data2$Signal_pred[i] = 1
     } else {
       check_data2$Signal_pred[i] = 0
     }
}
confusionMatrix(table(check_data2$Signal_pred, check_data2$Signal))
