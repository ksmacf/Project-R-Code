...
load("/home/user1/Desktop/Project-Environment.RData")
```
...
distribution(check_data1, "Distribution of predicted events with $\\tilde{t} = 1.2$ TeV and $ \\chi_1^0 = 600 $ GeV")
...
# Save tour history
t1_tr <- save_history(var1_tr, guided_tour(ashapetour(cl = col_v1_tr, alpha = 0.4, hull_var = c("black")), search_f = tourr:::search_better, alpha = pi/2, cooling = 0.5), step_size = Inf)
...
distribution(check_data2, "Distribution of predicted events with $\\tilde{t} = 1.225$ TeV and $ \\chi_1^0 = 400 $ GeV")
# Create Confusion Matrix based on the best possible cut-off seen in the plot (roughly 100 Events or less with Backgrounds is good I think)
check_data2 <- assign_label(check_data2, 0.5)
...
# False positive = Green/2 and False negative = red/3
write_csv(as.data.frame(var2), "/home/user1/Desktop/var2.csv")
...
htmlwidgets::saveWidget(as_widget(create_animation(t2, "/home/user1/Desktop/var2.csv", var2)), "/home/user1/Desktop/Images for thesis/tourr/bm2.html")
...
write_csv(as.data.frame(var2_tr), "/home/user1/Desktop/var2_tr.csv")
# Rescale
...
htmlwidgets::saveWidget(as_widget(create_animation_tr(t2_tr, "/home/user1/Desktop/var2_tr.csv", var2_tr)), "/home/user1/Desktop/Images for thesis/tourr/bm2_tr.html")
...
3) Benchmark 3 ($\\tilde{t} = 1.25$ TeV and $ \\chi_1^0 = 100 $ GeV)
```{r, message = False}
...
distribution(check_data3, "Distribution of predicted events with $\\tilde{t} = 1.25$ TeV and $ \\chi_1^0 = 100 $ GeV")
...
# False positive = Green/2 and False negative = red/3
write_csv(as.data.frame(var3), "/home/user1/Desktop/var3.csv")
...
htmlwidgets::saveWidget(as_widget(create_animation(t3, "/home/user1/Desktop/var3.csv", var3)), "/home/user1/Desktop/Images for thesis/tourr/bm3_2_2.html")
...
write_csv(as.data.frame(var3_tr), "/home/user1/Desktop/var3_tr.csv")
# Rescale
...
X11()
animate_xy(var3_tr, planned_tour(t3_tr), col = col_v3_tr, fps = 50)
X11()

htmlwidgets::saveWidget(as_widget(create_animation_tr(t3_tr, "/home/user1/Desktop/var3_tr.csv", var3_tr)), "/home/user1/Desktop/Images for thesis/tourr/bm3_tr.html")
...
distribution(check, "Distribution of predicted events with $\\tilde{t} = 750$ GeV and $ \\chi_1^0 = 1 $ GeV - Preliminary")
check <- assign_label(check, 0.6)
confusionMatrix(table(check$Signal_pred, check$Signal)) # Showed 0.9494 accuracy
rm(check)
...
distribution(check_data_I, "Distribution of predicted events with $\\tilde{t} = 750$ GeV and $ \\chi_1^0 = 1 $ GeV")
...
For the training set
```{r}
var_I_tr <- true_split_I[[1]] %>% select("MissingET.MET", "MissingET.Phi", "Lepton.PT", "Lepton.Phi", "ScalarHT.HT", "Jet1.PT", "BJet.PT", "Signal", "Signal_pred") %>% sample_n(NROW(true_split_I[[1]])/10)

col_v_I_tr <- if_else(var1_tr$Signal==0, "black", "blue")
...
X11()
animate_xy(var_I_tr, planned_tour(t1_tr), col = col_v_I_tr, fps = 50)
X11()
...
```{r}
  tour_path <- interpolate(t3, 0.1)
  d <- dim(tour_path)
  var_std <- var3#[order(as.data.frame(var3)$Signal_pred, decreasing = FALSE),]
  var <- read_csv("/home/user1/Desktop/var3.csv") # enter filename
  ...
  df <- as_tibble(mydat) %>% 
    mutate(Signal = rep(var$Signal_pred, d[3]))
  ...
  p <- ggplot(data = dfaxes, aes(frame = indx)) +
    ...
    geom_point(data = df, aes(x = d1, y = d2, colour=Signal), size=1) +
    scale_colour_manual(values = c("red", "green", "black", "blue")) +
    #geom_text(data=dfaxes_mat, aes(x=xloc, y=yloc, label=coef, frame = indx)) + 
    theme_void() +
    coord_fixed() +
...
```{r}
  tour_path <- interpolate(t1_tr, 0.1)
  d <- dim(tour_path)
  var_std <- var1_tr #[order(as.data.frame(variable)$Signal_pred, decreasing = FALSE),] 
  # Remove the brackets when don't want to order
  var <- read_csv("/home/user1/Desktop/var1_tr.csv") # enter filename
  ...
  colnames(myaxes)[5] <- "indx"
  df <- as_tibble(mydat) %>% 
    mutate(Signal = rep(var$Signal, d[3]))
  dfaxes <- as_tibble(myaxes) %>%
    mutate(labels=rep(colnames(var[,-ncol(var)]), d[3]))
  numbers <- round(seq(-0.2, 0.2, 0.4/(ncol(var)-1)), 2) # do this to match dimencions in yloc
  dfaxes_mat <- dfaxes %>%
