...
animation_highlighted <- function(bases, filename, variable){
  ...
  df_sp1 <- df[df[, "Prediction"]=="FN",] # Filter out False Negatives
  df_sp2 <- df[df[, "Prediction"]=="FP",] # Filter out False Positives
  p <- ggplot(data = dfaxes, aes(frame = indx)) +
    ...
    geom_point(data = df, 
               ...
               size=1, alpha = 0.1) + # change alpha to change transparency of points
    scale_colour_manual(values = c("red", "blue", "blue", "red")) +
    geom_point(data=df_sp1, 
               aes(x = d1, y = d2,colour = Prediction,
                   text=paste("Event Number:", df_sp1$Event_Number)), 
               alpha = 1) +
    geom_point(data=df_sp2, 
               aes(x = d1, y = d2, colour = Prediction,
                   text=paste("Event Number:", df_sp2$Event_Number)), 
               alpha = 1) +
    theme_void() +
    coord_fixed() +
