...
create_animation_tr <- function(bases, filename, variable){
  ...
  # Put label on 
  var["Signal"] <- if_else(var[,"Signal"]=="0", "Background", "Signal") 
  
  var <- var #%>% arrange(Signal_pred)
  mydat <- NULL; myaxes <- NULL
  
  for (i in 1:d[3]) {
    fp <- as.matrix(var_std) %*% matrix(tour_path[,,i], ncol=2)
    fp <- tourr::center(fp)
    entry <- matrix(1:nrow(var_std), ncol = 1)
    fp <- cbind(fp, entry)
    colnames(fp) <- c("d1", "d2", "Event_Number")
    mydat <- rbind(mydat, cbind(fp, rep(i, nrow(fp))))
    fa <- cbind(matrix(0, (ncol(var_std)), 2), matrix(tour_path[,,i], ncol=2))
  ...
  df <- as_tibble(mydat) %>% 
    mutate(Type = rep(var$Signal, d[3]))
  ...
  p <- ggplot(data = dfaxes, aes(frame = indx)) +
    ...
    geom_point(data = df, 
               aes(x = d1, y = d2, colour=Type, text=paste("Event Number:", df$Event_Number)), 
               size=1, alpha = 0.75) +
